...
function isMainView(img){
    ...
    point = images.findColor(img, "#5bcefe", {//蓝色的经验条
        region: [PHONE.height*0.05, PHONE.width*0.03, PHONE.height*0.15, PHONE.width*0.07],//一小块区域
        threshold: 4//完全匹配
    }); 
    if(logFlag) log("isMainView:"+point);
    if(point==null) {//有一个没找到就不找了
        return false;
    }else{
        return true;
    }
...
function isNextView(img){//下一步按钮
    if(img.height==PHONE.height) return;
    point = images.findColor(img, "#255a83", {//蓝色
        region: [PHONE.height*0.6, PHONE.width*0.7, PHONE.height*0.2, PHONE.width*0.2],//一小块区域
        threshold: 8//完全匹配
    }); 
    if(logFlag) log("isNextView:"+point);
    if(point==null) {//有一个没找到就不找了
        return false;
    }else{
        点击屏幕(point.x,point.y);
        return true;
    }
}
...
function isChuangView(img){//闯关按钮
    if(img.height==PHONE.height) return;
    point = images.findColor(img, "#8e6d3b", {//黄色
        region: [PHONE.height*0.6, PHONE.width*0.7, PHONE.height*0.2, PHONE.width*0.2],//一小块区域
        threshold: 8//完全匹配
    }); 
    if(logFlag) log("isChuangView:"+point);
    if(point==null) {//有一个没找到就不找了
        return false;
    }else{
        点击屏幕(point.x,point.y);
        ...
        while(1){
            ...
            if( isMainView(img) ){
                ...
                if( isChuangView(img) ){ //闯关按钮
                    ...
                    if(logFlag) log("闯关:"+runcount);
                }
            }else{
                if(logFlag) log("主界面:"+runcount);
            }
